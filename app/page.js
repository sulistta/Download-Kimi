...
import {
  Download, Monitor, Terminal, ChevronDown, ChevronRight,
...
function getLinuxAssets(assets) {
...
function getPrimaryAsset(assets, os) {
...
function parseChangelog(body) {
  ...
  for (const line of lines) {
    ...
    if (itemMatch && currentSection) {
      ...
      if (!sections.find(s => s.title === 'Notes')) {
        currentSection = { title: 'Notes', items: [] };
      } else {
        currentSection = sections.find(s => s.title === 'Notes');
      }
      currentSection.items.push(itemMatch[1].trim());
    }
  }
...
function countPlatformAssets(assets) {
  let win = 0, linux = 0;
  (assets || []).forEach(a => {
...
function DownloadButton({ asset, os, variant = 'primary', label }) {
  ...
  return (
    ...
    >
      {os === 'windows' ? <Monitor className={isPrimary ? 'w-5 h-5' : 'w-4 h-4'} /> : <Terminal className={isPrimary ? 'w-5 h-5' : 'w-4 h-4'} />}
      <span>{label || (os === 'windows' ? 'Download for Windows' : 'Download for Linux')}</span>
...
function HeroSection({ release, os, scrollToChangelog }) {
          ...
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8">
            {os === 'macos' ? (
              <div className="glass-card rounded-2xl px-8 py-4 text-center">
                ...
                <p className="text-zinc-500 text-sm">KimiTV Beta is available for Windows and Linux</p>
              </div>
            ) : os === 'unknown' ? (
              <>
                {windowsAsset && <DownloadButton asset={windowsAsset} os="windows" variant="primary" label="Download for Windows" />}
                {(appimage || deb) && <DownloadButton asset={appimage || deb} os="linux" variant={windowsAsset ? 'secondary' : 'primary'} label="Download for Linux" />}
              </>
            ) : (
              <>
                <DownloadButton asset={primaryAsset} os={os} variant="primary" />
                {os === 'windows' && (appimage || deb) && (
                  <DownloadButton asset={appimage || deb} os="linux" variant="secondary" label="Linux" />
                )}
                {os === 'linux' && windowsAsset && (
...
function ReleaseSpotlight({ release, os }) {
  if (!release) return null;
  const windowsAsset = getWindowsAsset(release.assets);
  const { appimage, deb } = getLinuxAssets(release.assets);
        ...
        <motion.div variants={scaleIn} className="glass-card-accent rounded-3xl p-8 sm:p-12 glow-accent">
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-80 space-y-3">
              ...
              {appimage && (
                <a href={appimage.browser_download_url} className="group flex items-center gap-4 p-4 rounded-2xl bg-zinc-900/60 border border-zinc-800 hover:border-cyan-500/30 transition-all duration-300 hover:bg-zinc-800/60">
                  <div className="w-10 h-10 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center shrink-0">
                    <Terminal className="w-5 h-5 text-orange-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-white">Linux (AppImage)</p>
                    <p className="text-xs text-zinc-500 truncate">{appimage.name}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs text-zinc-400 font-mono">{formatBytes(appimage.size)}</p>
                    <Download className="w-4 h-4 text-zinc-500 group-hover:text-cyan-400 transition-colors mt-1 ml-auto" />
                  </div>
                </a>
              )}
...
function PreviousReleasesSection({ releases }) {
                        ...
                        <div className="flex gap-2 mt-4">
                          {rel.assets.filter(a => !a.name.endsWith('.sig') && a.name !== 'latest.json').map(a => (
                            <a key={a.id} href={a.browser_download_url} className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-300 transition-colors border border-zinc-700">
                              <Download className="w-3 h-3" /> {a.name.endsWith('.exe') ? '.exe' : a.name.endsWith('.AppImage') ? '.AppImage' : a.name.endsWith('.deb') ? '.deb' : a.name.split('.').pop()}
...
function FAQSection() {
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: 'KimiTV Beta currently supports Windows (10/11, x64) and Linux (Ubuntu 22.04+, Debian-based, x64). macOS support is not currently available.'
    },
    {
